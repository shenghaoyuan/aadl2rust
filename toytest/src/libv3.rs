// last 自动生成的 Rust 代码 - 来自 AADL 模型 Toy_Example（共享变量）
// 生成时间: 2025-08-16
...
impl ToyExampleProc {
    pub fn new(cpu_id:isize) -> Self {
        // 创建共享变量（初值 0）
        let pos:PosShared = Arc::new(Mutex::new(0));

        // 线程持有共享引用
        let gnc = GncThread::new(pos.clone(), cpu_id);
        let tmtc = TmtcThread::new(pos.clone(), cpu_id);
...
pub struct ToyExampleSystem {
    ...
    // 你可以扩展为多进程并绑定不同 CPU
    pub bindings: Vec<(&'static str, isize)>, // (process_name, cpu_gnc, cpu_tmtc)
}
...
impl ToyExampleSystem {
    pub fn new() -> Self {
        ...
        Self {
            bindings: vec![("GNC_TMTC_POS", 0)],
        }
    }

    pub fn run(self) {
        for (proc_name, cpu_id) in self.bindings {
            match proc_name {
                "GNC_TMTC_POS" => {
                    let p = ToyExampleProc::new(cpu_id);
                    p.start();
                }
