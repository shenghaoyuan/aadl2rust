...
impl Process for speed_voterProcess {
    ...
    fn run(self: Self) -> () {
        let Self { wheel_sensor, wheel_sensorSend, laser_sensor, laser_sensorSend, speed, speedRece, speed_thr, .. } = self;
        thread::Builder::new()
    ...
    fn run(mut self) -> () {
        ...
        loop {
            ...
            };
            {
                ...
                loop {
                    match state {
                        State::s0 if 0 < wheel_sensor => {
                            speed_value = wheel_sensor;
                            state = State::s1;
                            continue;
                        },
                        State::s1 if 0 < laser_sensor => {
