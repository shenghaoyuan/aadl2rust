...
pub struct cameraDevice {
    pub picture: Option<Sender<picture>>,// Port: picture Out
...
impl Device for cameraDevice {
    ...
    fn run(self: Self) -> () {
        ...
        loop {
            ...
            let picture_val = please customize;
...
pub struct radarDevice {
    pub distance_estimate: Option<Sender<obstacle_position>>,// Port: distance_estimate Out
...
impl Device for radarDevice {
    ...
    fn run(self: Self) -> () {
        ...
        loop {
            let start = Instant::now();
            let distance_estimate_val = please customize;
...
pub struct speed_wheel_sensorDevice {
    pub speed: Option<Sender<speed>>,// Port: speed Out
    pub period_ms: u64,// 周期：1000ms
}
...
impl Device for speed_wheel_sensorDevice {
    ...
    fn run(self: Self) -> () {
        ...
        loop {
            let start = Instant::now();
            let speed_val = please customize;
...
pub struct brakeDevice {
    pub cmd: Option<Receiver<brake_cmd>>,// Port: cmd In
...
pub struct accelerationDevice {
    pub cmd: Option<Receiver<speed_cmd>>,// Port: cmd In
    pub period_ms: u64,// 周期：100ms
}
...
pub struct panelDevice {
    pub increase_speed: Option<Sender<speed>>,// Port: increase_speed Out
    pub decrease_speed: Option<Sender<speed>>,// Port: decrease_speed Out
    pub period_ms: u64,// 周期：2000ms
}
...
pub struct screenDevice {
    pub tire_pressure: Option<Receiver<pressure>>,// Port: tire_pressure In
    pub desired_speed: Option<Receiver<speed>>,// Port: desired_speed In
    pub actual_speed: Option<Receiver<speed>>,// Port: actual_speed In
    pub warning: Option<Receiver<bool>>,// Port: warning In
    pub entertainment_infos: Option<Receiver<entertainment_infos>>,// Port: entertainment_infos In
    pub period_ms: u64,// 周期：2000ms
}
...
pub struct tpmsDevice {
    pub pressure: Option<Sender<pressure>>,// Port: pressure Out
    pub period_ms: u64,// 周期：2000ms
}
...
impl Device for tpmsDevice {
    ...
    fn run(self: Self) -> () {
        ...
        loop {
            let start = Instant::now();
            let pressure_val = please customize;
...
pub struct bluetooth_controllerDevice {
    pub music: Option<Sender<music>>,// Port: music Out
    pub contacts: Option<Sender<contacts>>,// Port: contacts Out
    pub period_ms: u64,// 周期：2000ms
}
...
pub struct speakerDevice {
    pub music: Option<Receiver<music>>,// Port: music In
    pub period_ms: u64,// 周期：2000ms
}
