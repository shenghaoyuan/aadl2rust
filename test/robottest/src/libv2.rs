...
pub mod collecte_donnee_spg {
    ...
    use super::{collecte_donnee, Alpha_Type};
    // Wrapper for C function collecte_donnee
    // Original AADL port: d_source
    pub fn send(d_source: &mut Alpha_Type) -> () {
...
pub mod action_spg {
    ...
    // C binding to: action
    // source_files: "robot.c"
    use super::{action, Alpha_Type};
    // Wrapper for C function action
    // Original AADL port: d_action
    pub fn receive(d_action: Alpha_Type) -> () {
        unsafe { action(d_action);
         };
    }
    
}
// Port handler for d_action
// Direction: In
pub fn handle_d_action(port: Option<mpsc::Receiver<Alpha_Type>>) -> () {
    // Handle port: d_action;
}
...
pub struct capteurThread {
    // Port: evenement Out
    pub evenement: Option<mpsc::Sender<Alpha_Type>>,
    // 结构体新增 CPU ID
    pub cpu_id: isize,
...
pub struct controleThread {
    ...
    pub info_capteur: Option<mpsc::Receiver<Alpha_Type>>,
    // Port: comm_servo Out
    pub comm_servo: Option<mpsc::Sender<Alpha_Type>>,
    // 结构体新增 CPU ID
    pub cpu_id: isize,
...
pub struct servomoteurThread {
    // Port: ordre In
    pub ordre: Option<mpsc::Receiver<Alpha_Type>>,
    // 结构体新增 CPU ID
    pub cpu_id: isize,
...
pub struct p_capteurProcess {
    ...
    pub cpu_id: isize,
    // 内部端口: evenement Out
    pub evenementRece: Option<mpsc::Receiver<Alpha_Type>>,
...
pub struct p_controleProcess {
    ...
    pub info_capteur_droit: Option<mpsc::Receiver<Alpha_Type>>,
    // Port: comm_servo_droit Out
    pub comm_servo_droit: Option<mpsc::Sender<Alpha_Type>>,
    // Port: info_capteur_gauche In
    pub info_capteur_gauche: Option<mpsc::Receiver<Alpha_Type>>,
    // Port: comm_servo_gauche Out
    pub comm_servo_gauche: Option<mpsc::Sender<Alpha_Type>>,
    // 进程 CPU ID
    pub cpu_id: isize,
    // 内部端口: info_capteur_droit In
    pub info_capteur_droitSend: Option<mpsc::Sender<Alpha_Type>>,
    // 内部端口: comm_servo_droit Out
    pub comm_servo_droitRece: Option<mpsc::Receiver<Alpha_Type>>,
    // 内部端口: info_capteur_gauche In
    pub info_capteur_gaucheSend: Option<mpsc::Sender<Alpha_Type>>,
    // 内部端口: comm_servo_gauche Out
    pub comm_servo_gaucheRece: Option<mpsc::Receiver<Alpha_Type>>,
...
pub struct p_servomoteurProcess {
    ...
    pub cpu_id: isize,
    // 内部端口: ordre In
    pub ordreSend: Option<mpsc::Sender<Alpha_Type>>,
...
impl p_controleProcess {
    ...
    pub fn start(self: Self) -> () {
        ...
        let comm_servo_droit_rx = comm_servo_droitRece.unwrap();
        let info_capteur_gauche_rx = info_capteur_gauche.unwrap();
        let comm_servo_gauche_rx = comm_servo_gaucheRece.unwrap();
        std::thread::spawn(move|| {
            loop {
                ...
                if let Ok(msg) = comm_servo_droit_rx.try_recv() {
                ...
                if let Ok(msg) = comm_servo_gauche_rx.try_recv() {
