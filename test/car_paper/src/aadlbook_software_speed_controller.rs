...
use tokio::sync::broadcast::{Receiver as BcReceiver, Sender as BcSender};
...
pub struct speed_controllerProcess {
    ...
    pub obstacle_positionSend1: Option<Sender<bool>>, // 内部端口: obstacle_position In
    pub obstacle_positionSend2: Option<Sender<bool>>,
    pub obstacle_positionSend3: Option<Sender<bool>>,

    pub current_speedSend1: Option<Sender<u16>>, // 内部端口: current_speed In
    pub current_speedSend2: Option<Sender<u16>>,
    pub current_speedSend3: Option<Sender<u16>>,

    pub desired_speedSend1: Option<Sender<u16>>, // 内部端口: desired_speed In
    pub desired_speedSend2: Option<Sender<u16>>,
    pub desired_speedSend3: Option<Sender<u16>>,
...
impl Process for speed_controllerProcess {
    ...
    fn new(cpu_id: isize) -> Self {
        let mut accel_thr: speed_controller_accel_thrThread =
            speed_controller_accel_thrThread::new(cpu_id);
        let mut brake_thr: speed_controller_brake_thrThread =
            speed_controller_brake_thrThread::new(cpu_id);
        let mut warning_thr: speed_controller_warning_thrThread =
            speed_controller_warning_thrThread::new(cpu_id);
        let mut obstacle_positionSend1 = None;
        let mut obstacle_positionSend2 = None;
        let mut obstacle_positionSend3 = None;
        let mut current_speedSend1 = None;
        let mut current_speedSend2 = None;
        let mut current_speedSend3 = None;
        let mut desired_speedSend1 = None;
        let mut desired_speedSend2 = None;
        let mut desired_speedSend3 = None;

        let mut brake_cmdRece = None;
        let mut speed_cmdRece = None;
        let mut warningRece = None;
        let channel = crossbeam_channel::unbounded();
        obstacle_positionSend1 = Some(channel.0);
        // build connection:
        accel_thr.obstacle_position = Some(channel.1);
        let channel = crossbeam_channel::unbounded();
        obstacle_positionSend2 = Some(channel.0);
        // build connection:
        brake_thr.obstacle_position = Some(channel.1);
        let channel = crossbeam_channel::unbounded();
        obstacle_positionSend3 = Some(channel.0);
        // build connection:
        warning_thr.obstacle_position = Some(channel.1);

        let channel = crossbeam_channel::unbounded();
        current_speedSend1 = Some(channel.0);
        // build connection:
        accel_thr.current_speed = Some(channel.1);
        let channel = crossbeam_channel::unbounded();
        current_speedSend2 = Some(channel.0);
        // build connection:
        brake_thr.current_speed = Some(channel.1);
        let channel = crossbeam_channel::unbounded();
        current_speedSend3 = Some(channel.0);
        // build connection:
        warning_thr.current_speed = Some(channel.1);

        let channel = crossbeam_channel::unbounded();
        desired_speedSend1 = Some(channel.0);
        // build connection:
        accel_thr.desired_speed = Some(channel.1);
        let channel = crossbeam_channel::unbounded();
        desired_speedSend2 = Some(channel.0);
        // build connection:
        brake_thr.desired_speed = Some(channel.1);
        let channel = crossbeam_channel::unbounded();
        desired_speedSend3 = Some(channel.0);
        // build connection:
        ...
        return Self {
            ...
            obstacle_positionSend1,
            obstacle_positionSend2,
            obstacle_positionSend3,
            current_speed: None,
            current_speedSend1,
            current_speedSend2,
            current_speedSend3,
    ...
    fn start(self: Self) -> () {
        ...
        thread::Builder::new()
            ...
            .spawn(|| warning_thr.run())
            .unwrap();
        let obstacle_position_rx = obstacle_position.unwrap();
        thread::Builder::new()
            .name("data_forwarder_obstacle_position".to_string())
            .spawn(move || loop {
                if let Ok(msg) = obstacle_position_rx.recv() {
                    if let Some(tx) = &obstacle_positionSend1 {
                        let _ = tx.send(msg);
                    };
                    if let Some(tx) = &obstacle_positionSend2 {
                        let _ = tx.send(msg);
                    };
                    if let Some(tx) = &obstacle_positionSend3 {
                        let _ = tx.send(msg);
                    };
                };
        ...
        thread::Builder::new()
            ...
            .spawn(move || loop {
                if let Ok(msg) = current_speed_rx.try_recv() {
                    if let Some(tx) = &current_speedSend1 {
                        let _ = tx.send(msg);
                    };
                    if let Some(tx) = &current_speedSend2 {
                        let _ = tx.send(msg);
                    };
                    if let Some(tx) = &current_speedSend3 {
                        let _ = tx.send(msg);
                    };
                };
                std::thread::sleep(std::time::Duration::from_millis(1));
        ...
        thread::Builder::new()
            ...
            .spawn(move || loop {
                if let Ok(msg) = desired_speed_rx.try_recv() {
                    if let Some(tx) = &desired_speedSend1 {
                        let _ = tx.send(msg);
                    };
                    if let Some(tx) = &desired_speedSend2 {
                        let _ = tx.send(msg);
                    };
                    if let Some(tx) = &desired_speedSend3 {
                        let _ = tx.send(msg);
                    };
                };
                std::thread::sleep(std::time::Duration::from_millis(1));
                    ...
                    match state {
                        State::s0 if obstacle_position == true => {
                            if let Some(sender) = &self.brake_cmd {
                                println!("Tid=[{}]ctrl[T]:Issue a braking command",get_tid());
                                let _ = sender.send(1);
                            };
                            state = State::s0;
                            // complete,需要停
                        ...
                        State::s0 => {
                            // 理论上不会执行到这里，但编译器需要这个分支
                            println!("Tid=[{}]ctrl[T]:No braking command",get_tid());
                            break;
                        }
                    };
                    break;
...
impl Thread for speed_controller_warning_thrThread {
    ...
    fn run(mut self) -> () {
        ...
        loop {
            ...
            let start = Instant::now();
            let obstacle_position_val = self
                .obstacle_position
                .as_ref()
                .and_then(|rx| rx.try_recv().ok())
                .unwrap_or_else(|| Default::default());
