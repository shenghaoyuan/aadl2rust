...
impl Thread for controleThread {
    ...
    fn run(mut self) -> () {
        ...
        let mut comm_false: bool = false;
        let mut comm_true : bool = true;
        ...
        loop {
            ...
            let info_capteur:bool = match &self.info_capteur {
                Some(rx) => {
                    match rx.recv() {
                        Ok(val) => {
                            val //received value
