...
impl capteurThread {
    ...
    pub fn run(mut self) -> () {
        ...
        let mut count1: i32 = 0; // BA variable
        let mut state = CapteurState::S0;
        loop {
...
impl controleThread {
    ...
    pub fn run(mut self) -> () {
        ...
        let period: std::time::Duration = Duration::from_millis(110);
        enum ControleState { SInline, S1, S2, SOutline };
        let mut state = ControleState::SInline;
        loop {
            let start = Instant::now();

            // 尝试接收 info_capteur
            let event_val = match &self.info_capteur {
        ...
        // --- BA 宏步执行 ---
            loop {
                match state {
                    ControleState::SInline => {
                        // on dispatch → s1
                        state = ControleState::S1;
                        continue; // s1 不是 complete，要继续
                    }
                    ControleState::S1 => {
                        if let Some(true) = event_val {
                            state = ControleState::SInline; // complete，停
                        } else {
                            state = ControleState::SOutline; // complete，停
                            if let Some(tx) = &self.comm_servo {
                                let _ = tx.send(false);
                            }
                        }
                    }
                    ControleState::SOutline => {
                        // on dispatch → s2
                        state = ControleState::S2;
                        continue; // s2 不是 complete，要继续
                    }
                    ControleState::S2 => {
                        if let Some(true) = event_val {
                            state = ControleState::SInline; // complete，停
