...
impl System for integrationSystem {
    ...
    // 启动进程线程（它们内部会再 spawn 各自的子线程）
    std::thread::spawn(|| self.image_acquisition.start());
    std::thread::spawn(|| self.obstacle_detection.start());
    std::thread::spawn(|| self.panel_controller.start());
    std::thread::spawn(|| self.speed_voter.start());
    std::thread::spawn(|| self.speed_ctrl.start());
    std::thread::spawn(|| self.entertainment.start());
