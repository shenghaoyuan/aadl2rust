...
pub struct entertainment_thrThread {
    ...
    pub contacts: Option<Receiver<i8>>,// Port: contacts In
    pub infos: Option<Sender<i8>>,// Port: infos Out
    pub music_out: Option<Sender<bool>>,// Port: music_out Outs
...
impl Thread for entertainment_thrThread {
    ...
    fn run(mut self) -> () {
        ...
        loop {
            ...
            {
                //线程内部的应用代码，用户手写
                if let Some(receiver_music_in) = &self.music_in {
                    match receiver_music_in.try_recv() {
                        Ok(val) => {
                            if let Some(sender_music_out) = &self.music_out {
                                let _ = sender_music_out.send(val);
                            }
                            println!("thread:music_in_val: {}", val);
                        },
                        Err(e) => {
                            println!("error: {}", e);
                        }
                ...
                if let Some(receiver_contacts) = &self.contacts {
                    match receiver_contacts.try_recv() {
                        Ok(val) => {
                            if let Some(sender_infos) = &self.infos {
                                let _ = sender_infos.send(val);
                            }
                            println!("thread:contacts_val: {}", val);
                        },
                        Err(e) => {
                            println!("error: {}", e);
                        }
